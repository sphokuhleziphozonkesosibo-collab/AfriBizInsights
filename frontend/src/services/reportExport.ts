...
// 1. Executive PDF / Print Report Generator
      ...
      <body>
        <div class="header">
          <div>
            ...
            <p style="font-size: 13px; color: #64748b; margin: 4px 0 0 0;">Store: <strong>${user?.businessName || 'Mzansi Store'}</strong> • Owner: ${user?.fullName || 'Owner'}</p>
          </div>
...
// 2. WhatsApp Formatted Daily Summary Generator
export const generateWhatsAppSummary = (
  ...
  const lowStockItems = forecasts.filter((f) => f.stockoutWarning || f.currentStock <= 10);

  let message = `📊 *${user?.businessName || 'My Business'} — Daily Executive Recap*\n`;
  message += `📅 Date: ${today}\n\n`;

  message += `💰 *Financial Telemetry:*\n`;
  message += `• *Gross Revenue:* ${currencySymbol} ${summary ? summary.totalRevenue.toLocaleString() : '0'}\n`;
  message += `• *Operating Costs:* - ${currencySymbol} ${summary ? summary.totalExpenses.toLocaleString() : '0'}\n`;
  message += `• *True Net Profit:* ${currencySymbol} ${summary ? summary.netProfit.toLocaleString() : '0'} (${summary?.netMarginPercentage ?? 0}% Margin)\n`;
  message += `• *Total Orders:* ${summary?.totalOrders ?? 0} (Avg: ${currencySymbol} ${summary?.averageOrderValue ?? 0})\n`;
  message += `• *Customer Loyalty:* ${summary?.repeatCustomerPercentage ?? 0}% Repeat Buyers\n\n`;

  if (lowStockItems.length > 0) {
    message += `⚠️ *Inventory & Stockout Warnings:*\n`;
    lowStockItems.slice(0, 3).forEach((item) => {
      message += `• *${item.productName}:* ${item.currentStock} left (Runout in ~${item.stockoutInDays ?? 0}d) ➔ Order +${item.recommendedReorderQty}\n`;
    });
    message += `\n`;
  } else {
    message += `✨ *Inventory Health:* All product stock levels healthy (30+ days buffer).\n\n`;
  }

  message += `_Generated by AfriBiz Insights 🌍_`;
  return message;
};

// 3. One-Click Complete CSV Backup Download
export const downloadStoreBackupCsv = (
  user: AuthUser | null,
  summary: DashboardSummary | null,
  topProducts: TopProduct[],
  deadStock: DeadStockProduct[]
) => {
  let csv = `AFRIBIZ INSIGHTS STORE DATA BACKUP\n`;
  csv += `Store,${user?.businessName || 'My Business'}\n`;
  csv += `Currency,${user?.currency || 'ZAR'}\n`;
  csv += `Export Date,${new Date().toISOString()}\n\n`;

  csv += `FINANCIAL SUMMARY\n`;
  csv += `Gross Sales Revenue,${summary?.totalRevenue ?? 0}\n`;
  csv += `Cost of Goods Sold (COGS),${summary?.costOfGoodsSold ?? 0}\n`;
  csv += `Operating Expenses,${summary?.totalExpenses ?? 0}\n`;
  csv += `True Net Profit,${summary?.netProfit ?? 0}\n`;
  csv += `Gross Margin %,${summary?.grossMarginPercentage ?? 0}%\n`;
  csv += `Net Margin %,${summary?.netMarginPercentage ?? 0}%\n`;
  csv += `Total Transactions,${summary?.totalOrders ?? 0}\n\n`;

  csv += `TOP SELLING PRODUCTS\n`;
  csv += `Product Name,Category,Units Sold,Revenue Generated\n`;
  topProducts.forEach((p) => {
    csv += `"${p.name}","${p.category || 'General'}",${p.totalUnitsSold},${p.totalRevenueGenerated}\n`;
  });
  csv += `\n`;

  if (deadStock.length > 0) {
    csv += `DEAD STOCK / TRAPPED CAPITAL\n`;
    csv += `Product Name,Category,Current Stock,Trapped Cash Value\n`;
    deadStock.forEach((d) => {
      csv += `"${d.name}","${d.category || 'General'}",${d.currentStock},${d.trappedCapital}\n`;
    });
  }
  ...
  link.setAttribute(
    'download',
    `AfriBiz_Backup_${user?.businessName?.replace(/\s+/g, '_') || 'Store'}_${new Date().toISOString().slice(0, 10)}.csv`
  );
